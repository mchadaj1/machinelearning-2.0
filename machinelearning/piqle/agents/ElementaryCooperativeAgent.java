...
import environment.ElementaryStateWithActions;
...
abstract public class ElementaryCooperativeAgent<F extends ElementaryStateWithActions>
extends ElementaryAgent {
	...
	protected ArrayList<NullableAction> oldActions=new ArrayList<NullableAction>();
	...
	protected F buildState(IEnvironment universe,IState s,ArrayList<NullableAction> old){
	...
	{
		return NullableAction.getNullAction();
	}
	
	public void newEpisode() {
		super.newEpisode(); 
		this.lastAction=NullableAction.getNullAction();
	}
	
	public void addNeighbour(ElementaryCooperativeAgent n)
	{
		this.neighbours.add(n);
		oldActions.add((NullableAction)n.getLastAction());
	}
	
	// TODO For debug and setting : to be erased when verification completed
	public String neighboursActions(){
		String s=""; 
		int i=0; 
		Iterator<ElementaryCooperativeAgent>cursor=this.neighbours.iterator();
		while(cursor.hasNext()){
				ElementaryCooperativeAgent ag=cursor.next(); 
				IAction ac=ag.getLastAction();
				s+="Action neighbour number "+i+":"+ac+"\n"; 
		}
		return s; 

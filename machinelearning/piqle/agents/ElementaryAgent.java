...
public class ElementaryAgent extends LoneAgent{
 ...
 /** Place the agent  in the environment.*/
    ...
    public void setSelector(ISelector al){
    	this.algorithm=al; 
    }
    
    public ElementaryAgent(IEnvironment s,Filter f){
    	super((IEnvironmentSingle)s,null);
    	// TODO : put a random selector ?
    	System.err.println("Don't forget to set the ISelector !");
    	this.fil=f; 
    }
    
    public ElementaryAgent(IEnvironment s, ISelector al){ 
    	super((IEnvironmentSingle) s,al); 
    	this.fil=new IdentityFilter();  
        }

...
abstract public class AbstractMemorySelector implements ISelector {
    ...
    /*
	/* getStore
	 * @return Returns the current reward store
	 */
	public IRewardStore getRewardStore() {
		return this.memory;
    }
	...
	public double getValue(IState s, IAction a) {
		return this.memory.get(s, a);
	}
    /* setValue
     * @param s1 Previous State
     * @param s2 Current State
     * @param a Action
     * @param QSA New QSA Value
     */
     
    public void setValue(IState s1,IState s2,IAction a,double qsa ) {
	    
	    this.memory.put(s1, a, s2, qsa);
    }
	//*======================================================================================================    
	...
	public void setGeometricAlphaDecay() {
		this.geometricDecay = true;
	}

	public void setExponentialAlphaDecay() {
		this.geometricDecay = false;
	}
	...
	public boolean getGeometricDecay() {
		return this.geometricDecay;
	}

	public void setGeometricDecay(boolean gd) {
		this.geometricDecay=gd;
	}
	...
	public void learn(IState s1, IState s2, IAction a, double reward) {
		if (geometricDecay)
			this.alpha *= decayAlpha;
		else {
			this.alpha = 1 / Math.pow(count + 1.0, this.alphaDecayPower);
		}
		...
		if (la.size() != 0) {
			...
			qsa += this.alpha * (reward + gamma * maxqsap - qsa);
           	memory.put(s1, a, s2, qsa);
		} else {
			memory.put(s1, a, s2, qsa + this.alpha * (reward - qsa));
				}
	}
